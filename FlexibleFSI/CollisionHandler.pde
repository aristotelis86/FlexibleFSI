...
class CollisionHandler {
  ...
  float e = 1; // wall bouncing coefficient
  float f = 0; // wall friction coefficient
  ...
  //================= Method ====================//
  // Set wall bouncing coefficient
  void setBounce( float e_ ) { e = e_; }
  // Set wall friction coefficient
  void setFriction( float f_ ) { f = f_; }
  
  // Detect Boundary Collisions
  void DetectBoundCollision() {
    for (int i=0; i<Ncp; i++) {
      ControlPoint cp = LocalCPoints.get(i);
      if (cp.position.x < cp.radius) {
        this.ResolveBoundCollisions( "West", cp );
      }
      else if (cp.position.x > cp.myWindow.x.inE - cp.radius) {
        this.ResolveBoundCollisions( "East", cp );
      }
      if (cp.position.y < cp.radius) {
        this.ResolveBoundCollisions( "North", cp );
      }
      else if (cp.position.y > cp.myWindow.y.inE - cp.radius) {
  ...
  void ResolveBoundCollisions( String bound, ControlPoint cp ){
    if ((bound.equals("North")==true) || (bound.equals("South")==true)) {
      float vx = (1-f)*cp.velocity.x;
      float vy = -e*cp.velocity.y;
      float x = cp.position.x;
      float y = cp.position.y;
      if (bound.equals("North")==true) y = cp.radius;
      else if (bound.equals("South")==true) y = cp.myWindow.y.inE-cp.radius;
      cp.setPosition(x,y); cp.setVelocity(vx,vy);
    }
    if ((bound.equals("West")==true) || (bound.equals("East")==true)) {
      float vx = -e*cp.velocity.x;
      float vy = (1-f)*cp.velocity.y;
      float x = cp.position.x;
      float y = cp.position.y;
      if (bound.equals("West")==true) x = cp.radius;
      else if (bound.equals("East")==true) x = cp.myWindow.x.inE-cp.radius;
      cp.setPosition(x,y); cp.setVelocity(vx,vy);
    }
  }
